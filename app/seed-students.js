...
const SERVICE_ROLE_KEY = 'PEGA_AQUI_TU_SERVICE_ROLE_KEY'; // Debe ser la clave service_role (secret), NO la anon_key
...
async function seed() {
  console.log('Iniciando registro oficial de estudiantes...');

  for (const st of students) {
    // 1. Crear el usuario mediante la API de administración oficial de Supabase
    ...
    if (error) {
      console.error(`Error al registrar ${st.email}:`, error.message);
      continue;
    }

    // 2. Guardar su perfil en la tabla 'profiles'
    if (data.user) {
      ...
      console.log(`✅ Creado con éxito: ${st.email}`);
    }
  }

  console.log('¡Todos los estudiantes fueron registrados correctamente!');
}
